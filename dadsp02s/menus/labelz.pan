...
{if(not(menuinrefresh()), init_labelz_pan())}
...
<L>
     Height Label (Z) : < w{80} skip{1} >  ~noop(<{_zlabel}>)~ _zlab=1; _tmp = input(5); _zlabel = if(valuetype(_tmp)==4, _tmp, getzunits); _zlabel
...
~if(_zlab, if(getzunits != _zlabel, poff; setzlab(_zlabel); _replot = 1))
